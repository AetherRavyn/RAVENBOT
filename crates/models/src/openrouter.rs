...
impl OpenRouterProvider {
    ...
    async fn send_chat(
        ...
        let api_key = self.api_key.as_ref()
            .ok_or_else(|| ModelError::Auth("OpenRouter API key not configured".to_string()))?;
        ...
        let response = self.client
            ...
            .header("HTTP-Referer", "https://ravenbot.local")
            .header("X-Title", "RAVENBOT")
            .json(&request)
            .send()
            .await
...
#[async_trait]
impl ModelProviderTrait for OpenRouterProvider {
